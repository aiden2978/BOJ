...
for _ in range(E):
    ...
    if graph[i][j] == -1:
        graph[i][j], graph[j][i] = c, c
    else:
        graph[i][j], graph[j][i] = min(graph[i][j], c), min(graph[j][i], c)
...
if 10**8 + 1 not in [from_1[v1], from_v1[v2], from_v2[N], from_1[v2], from_v2[v1], from_v1[N]]:
    print(min(from_1[v1] + from_v1[v2] + from_v2[N], from_1[v2] + from_v2[v1] + from_v1[N]))
else:
    print(-1)